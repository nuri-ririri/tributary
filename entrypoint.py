...
def collect_engine_temperature():
    database = redis.Redis(host="redis", port=6379, db=0, decode_responses=True)
    engine_temperature_values = database.lrange(DATA_KEY, 0, -1)
    #but the above values are strings, therefore we need to convert them
    engine_temperatures = list(map(float, engine_temperature_values))

    #calculate the average_engine_temperature
    if engine_temperatures:
        average_engine_temperature = sum(engine_temperatures) / len(engine_temperatures)
        current_engine_temperature = engine_temperatures[0] #the item that is added the last and is located at the top of the list
        #print the results
        ...
        logger.info("No engine temperature data.")
        return{"success": False, "error": "No engine temperature data"}, 404
